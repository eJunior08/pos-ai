...
function makeContext(products, users) {
  ...
  const productAverageAgeNormalized = Object.fromEntries(
    products.map((product) => {
      const averageAgePerProduct = ageCounts[product.name]
        ? ageSums[product.name] / ageCounts[product.name]
        : midAge;

      return [product.name, normalize(averageAgePerProduct, minAge, maxAge)];
    }),
  );
  ...
  return {
    ...
    minPrice,
    maxPrice,
    qtyCategories: categories.length,
    qtyColors: colors.length,
...
// normaliza para tipos de dados como categoria ("tenis", "sandalia", etc)
...
function encodeProduct(product, context) {
  ...
  // pega a media de idade do produto
  // colocar 0.5 só para deixar um peso caso o usuario comprou um produto que não está ali
...
// retornar o perfil de compra desse usuario
// gerar um novo tensor a partir das compras do usuario
function encodeUser(user, context) {
  if (user.purchases.length) {
    return tf
      .stack(user.purchases.map((product) => encodeProduct(product, context)))
      .mean(0) // média desses vetores para retornar um unico valor
      .reshape([
        1, // uma linha
        context.dimentions, // colunas = dimentions
      ]);
  }
  ...
  return tf
    .concat1d([
      ...
      ]), // para usuario que nunca comprou, usar a idade
      tf.zeros([context.qtyCategories]), // categoria é ignorado
      tf.zeros([context.qtyColors]), // color é ignorado
    ])
...
async function configureNeuralNetAndTrain(trainData) {
  ...
  // Camada de entrada
  // - inputShape: Número de features por exemplo de treino (trainData.inputDim); numero de caracteristicas que vai usar
  //   Exemplo: Se o vetor produto + usuário = 20 números, então inputDim = 20
  // - units: 128 neurônios (muitos "olhos" para detectar padrões); calculos matematicos
  ...
  await model.fit(trainData.xs, trainData.ys, {
    epochs: 100, // qtd que o modelo vai rodar
    batchSize: 32, // vai fazer de 32 em 32 para não travar tudo
    shuffle: true, // embaralhar o dado para não ficar viciado
    callbacks: {
...
function recommend(user, ctx) {
  ...
  //    (preço ignorado, idade normalizada, categorias ignoradas) - para usuario que nunca comprou
